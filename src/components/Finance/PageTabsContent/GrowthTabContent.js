...
class GrowthTabTable extends Component {
    constructor(props) {
        super(props)
        this.state = {
            open: false,
    ...
    render() {
        ...
        dataSource = dataSource[0] ? dataSource[0].growth : null

        //columns
        const growthColumns = Object.keys(columnsNames.growth).map((k, i) => {
            if (k === 'operate') {
                return {
                    title: (<span style={{ color: '#1890ff' }}>
                        {columnsNames.growth[k]}
                    </span>),
                    dataIndex: k,
                    render: (text, record) => {
                        return (
...
const chartOption = {
    baseOption: {
        ...
        legend: {
            right: '0%',
            data: ["净利润增长率","收入增长率","总资产增长率","固定资产占比"],
            orient: 'vertical',
            align: 'left',
            top: '30%',
            itemGap: 20,
...
class GrowthTabChart extends Component {
    constructor(props) {
        super(props)
    ...
    drawChart = () => {
        ...
        barChart.setOption({
            baseOption: {
                ...
                yAxis: {
                    data: this.props.dataSource[0].growth.map(obj => obj.stkcd.match(/^\d{6}/)[0])
                },
            },
            options: this.props.dataSource.map(obj => {
                const mergeValues = obj.growth.reduce((o1, o2) => {
...
class GrowthTabTimeTable extends Component {
    ...
    render() {
        ...
        //columns
        const growthColumns = Object.keys(timeColumnsNames.growth).map((k, i) => {
            if (k === 'operate') {
                return {
                    title: (<span style={{ color: '#1890ff' }}>
                        {timeColumnsNames.growth[k]}
                    </span>),
                    dataIndex: k,
                    render: (text, record) => {
                        return (
...
class GrowthTabTimeChart extends Component {
    constructor(props) {
        super(props)
    ...
    drawChart = () => {
        ...
        lineBarChart.setOption({
            baseOption: {
                ...
                xAxis: {
                    data: this.props.dataSource[0].growth.map(obj => obj.date)
                },
            },
            options: this.props.dataSource.map(obj => {
                const mergeValues = obj.growth.reduce((o1, o2) => {
