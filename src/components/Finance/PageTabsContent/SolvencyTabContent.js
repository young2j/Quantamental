...
class SolvencyTabTable extends Component {
    constructor(props) {
        super(props)
        this.state = {
            open: false,
    ...
    render() {
        ...
        dataSource = dataSource[0] ? dataSource[0].solvency : null  //mock数据为了保证有数不出错
        const { blacksheet, follows } = this.props.userInfo

        //columns
        const solvencyColumns = Object.keys(columnsNames.solvency).map((k, i) => {
            if (k === 'operate') {
                return {
...
const chartOption = {
    baseOption: {
        ...
        legend: {
            right: '0%',
            data: ["流动比率","速动比率","现金比率","资产负债率","产权比率","利息保障倍数"],
            orient: 'vertical',
            align: 'left',
            top: '30%',
            itemGap: 20,
...
class SolvencyTabChart extends Component {
    constructor(props) {
        super(props)
    ...
    drawChart = () => {
        ...
        barChart.setOption({
            baseOption: {
                ...
                yAxis: {
                    data: this.props.dataSource[0].solvency.map(obj => obj.stkcd.match(/^\d{6}/)[0])
                },
            },
            options: this.props.dataSource.map(obj => {
                const mergeValues = obj.solvency.reduce((o1, o2) => {
...
class SolvencyTabTimeTable extends Component {
...
class SolvencyTabTimeChart extends Component {
    constructor(props) {
        super(props)
    ...
    drawChart = () => {
        ...
        lineBarChart.setOption({
            baseOption: {
                ...
                xAxis: {
                    data: this.props.dataSource[0].solvency.map(obj => obj.date)
                },
            },
            options: this.props.dataSource.map(obj => {
                const mergeValues = obj.solvency.reduce((o1, o2) => {
