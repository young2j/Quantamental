...
class OperationTabTable extends Component {
    constructor(props) {
        super(props)
        this.state = {
            open: false,
    ...
    render() {
        ...
        //columns
        const operationColumns = Object.keys(columnsNames.operation).map((k, i) => {
            if (k === 'operate') {
                return {
                    title: (<span style={{ color: '#1890ff' }}>
                        {columnsNames.operation[k]}
                    </span>),
                    dataIndex: k,
                    render: (text, record) => {
                        return (
...
const chartOption = {
    baseOption: {
        ...
        legend: {
            right: '0%',
            data: ["存货周转率","应收账款周转率","总资产周转率","固定资产周转率"],
            orient: 'vertical',
            align: 'left',
            top: '30%',
            itemGap: 20,
...
class OperationTabChart extends Component {
    constructor(props) {
        super(props)
    ...
    drawChart = () => {
        ...
        barChart.setOption({
            baseOption: {
                ...
                yAxis: {
                    data: this.props.dataSource[0].operation.map(obj => obj.stkcd.match(/^\d{6}/)[0])
                },
            },
            options: this.props.dataSource.map(obj => {
                const mergeValues = obj.operation.reduce((o1, o2) => {
...
class OperationTabTimeTable extends Component {
...
class OperationTabTimeChart extends Component {
    constructor(props) {
        super(props)
    ...
    drawChart = () => {
        ...
        lineBarChart.setOption({
            baseOption: {
                ...
                xAxis: {
                    data: this.props.dataSource[0].operation.map(obj => obj.date)
                },
            },
            options: this.props.dataSource.map(obj => {
                const mergeValues = obj.operation.reduce((o1, o2) => {
