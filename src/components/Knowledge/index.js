...
import { Form, Input, Button, DatePicker,Spin,message,Modal } from 'antd';
...
const ChapterDisplayView = (props)=>{
  ...
  return (
    <div id='chapter-form' style={{marginTop:0}}>
      <div className='title-item' style={{marginTop:20}}>
      ...
      <div ref={contentRef} style={{width:"77%",margin:'50px auto'}}></div>
    </div>
  )
}
...
class ChapterEditView extends Component {
  ...
  onOk = ()=>{
    this.editor.txt.html(this.props.formData.content)
    this.refs.formRef.setFieldsValue(this.props.formData)
    this.onFinish(this.props.formData)
  }
  ...
  render() {
    const {scrollTop,isLoading,visible} = this.state
    ...
    return (
      <Spin spinning={isLoading}>
        ...
        <div className={scrollTop? 'editor-form-fixtop':'editor-form'}>
          <Form.Item
            label=''
            name='content'
            rules={[
